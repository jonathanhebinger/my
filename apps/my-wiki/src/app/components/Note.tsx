...
import WikiPageEditor from './NoteEditor'
import WikiPageViewer from './NoteViewer'
...
export default function WikiPage({ page: initPage }: { page: NoteDto }) {
  const { update, remove } = useNotebookNoteContext()
  const { close } = useOpenedNoteList()
  ...
  const handleClose = () => close(page.id)
